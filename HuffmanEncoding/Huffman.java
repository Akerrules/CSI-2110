...
/**
 * Class Huffman that provides huffman compression encoding and decoding of
 * files
 * 
 * @author Aditya Kandel 2021
 *         PLEASE DO NOT REMOVE MY NAME FROM THE CODE
 *
 */
...
public class Huffman {
	...
	private class InBitStream {
		...
		public int readBit() throws IOException { // read one bit to Output Stream (using byte buffer)
			if (buffCount == 8) { // current buffer has already been read must bring next byte
				...
				if (buffer == -1)
					return -1; // indicates stream ended
			}
			...
			buffCount++;
			if ((aux & buffer) > 0)
				return 1; // this checks whether bit buffcount of buffer is 1
			else
				return 0;
	...
	private HuffmanTreeNode buildEncodingTree(ArrayList<Integer> freqTable) {
		...
		HeapPriorityQueue<Integer, HuffmanTreeNode> pQ = new HeapPriorityQueue<Integer, HuffmanTreeNode>();
		...
		while (pQ.size() > 1) {
			...
			HuffmanTreeNode newNode = new HuffmanTreeNode(0, 0, e1.getValue(), e2.getValue());
			pQ.insert(e1.getKey() + e2.getKey(), newNode);
		}
		Entry<Integer, HuffmanTreeNode> e = pQ.removeMin();
		return e.getValue();
	}
	...
	private ArrayList<String> buildEncodingTable(HuffmanTreeNode encodingTreeRoot) {
		...
		for (int i = 0; i < 257; i++)
			code.add(i, null);
		buildCode(encodingTreeRoot, code, "");
		return code;
	}

	private HuffmanTreeNode buildCode(HuffmanTreeNode root, ArrayList<String> array, String code) {
		if (root == null) {
			return null;
		}
		HuffmanTreeNode right = buildCode(root.right, array, code += "1");
		if (right == null) {
			code = code.substring(0, code.length() - 1);
		}
		if (root.right == null) {
			array.set(root.character, code);
		}
		return buildCode(root.left, array, code += "0");
	}
	...
	private void encodeData(InputStream input, ArrayList<String> encodingTable, OutputStream output)
		...
		/************ your code comes here ************/
		int Encodedbits = input.read();
		int numOfbyteInput = 0;
		int numOfbyteOutput = 0;

		while (true) {
			// get the special byte ecoding if no more input left
			String huffmancode = Encodedbits == -1 ? encodingTable.get(256) : encodingTable.get(Encodedbits);
			for (int i = 0; i < huffmancode.length(); i++) { //
				bitStream.writeBit(Integer.parseInt(huffmancode.substring(i, i + 1))); // write to text file
				numOfbyteOutput++; // track output size
			}
			if (Encodedbits == -1) // if we reached the end then break out of the loop
				break;
			Encodedbits = input.read(); // read the byte from the input
			numOfbyteInput++; // keep track of the input size;
		}
		bitStream.close(); // close bit stream; flushing what is in the bit buffer to output file

		System.out.println("\nNumber of bytes in input : " + numOfbyteInput);
		System.out.println("Number of bytes in output : " + (int) (Math.ceil(numOfbyteOutput / 8.0)));
	...
	private void decodeData(ObjectInputStream input, HuffmanTreeNode encodingTreeRoot, FileOutputStream output)
		...
		/************ your code comes here ************/
		int i = 0;
		HuffmanTreeNode curnode = encodingTreeRoot;
		int outputBit = 0;
		int inputBit = 0;
		...
		while (i != -1) {
			...
			inputBit++;
			curnode = i == 1 ? curnode.right : curnode.left; // go right if input is 1 else go left

			if (curnode.character == 256) { // break out of the loop if the 256th character is found
				break;
			}
			if (curnode.isLeaf()) { // write to output if node is a leaf
				output.write(curnode.getChar());
				outputBit++;
				curnode = encodingTreeRoot;
			}

		}
		System.out.println("Number of bytes in input : " + (int) (Math.ceil(inputBit / 8.0)));
		System.out.println("Number of bytes in output : " + outputBit);
